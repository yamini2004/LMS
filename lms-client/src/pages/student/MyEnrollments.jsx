        ...
        try {
          ...
          const tempProgressArray = await Promise.all(
            enrolledCourses.map(async (course)=>{
              //const {data} = await axios.post(`${backendUrl}/api/user/get-course-progress`,{headers: {Authorization: `Bearer ${token}`}})
          ...
          <tbody className='text-gray-700'>
            {enrolledCourses.map((course,index)=>(
              <tr key={index} className='border-b border-gray-500/20'>
                 <td className='md:px-14 pl-2 md:pl-4 py-3 flex items-center space-x-3'>
                  <img src={course.courseThumbnail} alt=" 1" className='w-14 sm:w-24 md:w-28'/>
